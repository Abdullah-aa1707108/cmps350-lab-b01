...
export class UnitConverter {
  constructor() {
    this.unitMaps = [
      ...
      new UnitMap('gram', 'pound', 0.0022046),
  ...
  kilogramToOunce(val) {
    const multiplier = this.unitMaps.find((u) => u.from === "gram" && u.to === 'ounce').multiplier;
    const convertedVal = (val * 1000) / multiplier;
    return this.round(convertedVal, 4);
  }

  kilogramToPound(val) {
    const multiplier = this.unitMaps.find((u) => u.from === "gram" && u.to === 'pound').multiplier;
    const convertedVal = (val * 1000) * multiplier;
    return this.round(convertedVal, 4);
  }
